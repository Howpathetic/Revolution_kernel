...
void __init shooter_u_allocate_fb_region(void)
{
	unsigned long size;
...
static int shooter_u_lcd_on(struct platform_device *pdev)
{
...
static void shooter_u_set_backlight(struct msm_fb_data_type *mfd)
{
	if (panel_type == PANEL_ID_SHR_SHARP_NT) {
...
static int mipi_shooter_u_device_register(const char* dev_name, struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	shooter_u_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &shooter_u_panel_data,
		sizeof(shooter_u_panel_data));
	if (ret) {
...
static int __init mipi_cmd_novatek_blue_qhd_pt_init(void)
{
	...
	ret = mipi_shooter_u_device_register("mipi_novatek", &pinfo, MIPI_DSI_PRIM,
...
void __init shooter_u_init_fb(void)
{
...
device_initcall_sync(shooter_u_panel_init);
